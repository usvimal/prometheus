...
def _check_task_already_completed(task_id: str) -> bool:
    """Check if a task has already completed (to avoid stale retry)."""
    try:
        result_path = DRIVE_ROOT / "task_results" / f"{task_id}.json"
        if result_path.exists():
            result = json.loads(result_path.read_text(encoding="utf-8"))
            if result.get("status") == "completed":
                return True
    except Exception:
        log.debug("Failed to check task result for %s", task_id, exc_info=True)
    return False


def _check_task_made_progress(task_id: str, task_type: str) -> bool:
    """Check if evolution/review task made any code/progress.
    
    Returns True if task made progress (should retry on timeout).
    Returns False if task is stuck (don't retry).
    """
    if task_type not in ("evolution", "review"):
        return True  # Always retry non-evolution tasks
    
    try:
        result_path = DRIVE_ROOT / "task_results" / f"{task_id}.json"
        if result_path.exists():
            result = json.loads(result_path.read_text(encoding="utf-8"))
            # Check for code changes: commits, file modifications, etc.
            result_text = str(result.get("result", "")).lower()
            # Patterns indicating actual progress
            progress_patterns = ["commit", "pushed", "implemented", "refactored", 
                                "fixed", "added", "updated", "created", "modified"]
            if any(p in result_text for p in progress_patterns):
                return True
            # Also check for non-trivial completion tokens (means work was done)
            total_rounds = int(result.get("total_rounds", 0))
            if total_rounds > 15:  # More than 15 rounds = substantial work
                return True
    except Exception:
        log.debug("Failed to check task progress for %s", task_id, exc_info=True)
    
    # No clear progress detected - don't retry (prevents infinite stuck loops)
    return False
...
def enforce_task_timeouts() -> None:
    ...
    for task_id, meta in list(RUNNING.items()):
        ...
        # CRITICAL FIX: Check if task already completed before spawning retry
        # This prevents the race condition where:
        # 1. Task completes successfully (status=completed in task_results/)
        # 2. RUNNING dict not immediately cleaned up
        # 3. 30 min later, enforce_task_timeouts sees task in RUNNING >1800s
        # 4. Without this check, it would spawn a retry anyway
        if _check_task_already_completed(task_id):
            log.info("Task %s already completed, cleaning up RUNNING entry (was stale)", task_id)
            RUNNING.pop(task_id, None)
            ...
            persist_queue_snapshot(reason="task_already_completed")
            continue

        # For evolution/review tasks: check if they made any progress
        # If stuck (no commits, no changes), don't retry - it's a pattern failure
        if not _check_task_made_progress(task_id, task_type):
            log.info("Task %s (%s) made no progress, not retrying (prevents infinite stuck loop)", 
                     task_id, task_type)
            append_jsonl(
                DRIVE_ROOT / "logs" / "supervisor.jsonl",
                {
                    "ts": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                    "type": "task_hard_timeout_no_retry",
            ...
            if owner_chat_id:
                send_with_budget(owner_chat_id, (
                    ...
                    f"Task made no progress (no code changes). Not retrying."
                ))
            RUNNING.pop(task_id, None)
            if worker_id in workers.WORKERS and workers.WORKERS[worker_id].busy_task_id == task_id:
                workers.WORKERS[worker_id].busy_task_id = None
            if worker_id in workers.WORKERS:
                workers.respawn_worker(worker_id)
            persist_queue_snapshot(reason="task_no_progress")
            continue
...
def enqueue_evolution_task_if_needed() -> None:
    ...
    if consecutive_failures >= 3:
        ...
        append_jsonl(
            ...
            {
                "ts": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                "type": "evolution_circuit_breaker",
                "consecutive_failures": consecutive_failures,
                "reason": "pausing evolution after 3 consecutive failures",
            },
        )
        if owner_chat_id:
            send_with_budget(
                int(owner_chat_id),
                f"🛑 Evolution paused: {consecutive_failures} consecutive failures.\n"
                f"Use /evolve to restart when ready.",
            )
        return
...
# ---------------------------------------------------------------------------
# Review + task completion handling
# ---------------------------------------------------------------------------

def on_task_completed(task_id: str, task_type: str, result_status: str) -> None:
    """Called when a task completes (success or failure).
    
    Updates evolution failure counter for circuit breaker.
    """
    ...
    if result_status == "completed":
        ...
        if st.get("evolution_consecutive_failures"):
        ...
        consecutive_failures = int(st.get("evolution_consecutive_failures") or 0) + 1
        st["evolution_consecutive_failures"] = consecutive_failures
        save_state(st)
        append_jsonl(
            DRIVE_ROOT / "logs" / "supervisor.jsonl",
            {
