...
def sync_runtime_dependencies(reason: str) -> Tuple[bool, str]:
    ...
    cmd: List[str] = [sys.executable, "-m", "pip", "install", "-q", "--break-system-packages"]
    source = ""
    if req_path.exists():
        cmd += ["-r", str(req_path)]
...
# ---------------------------------------------------------------------------
# Pre-push test helpers (for test_pre_push_runs_tests)
# ---------------------------------------------------------------------------

def get_staged_files() -> List[str]:
    """Return list of staged Python files."""
    rc, out, err = git_capture(["git", "diff", "--cached", "--name-only", "--diff-filter=ACM"])
    if rc != 0:
        return []
...
def get_diff_summary() -> str:
    ...
    for line in out.splitlines():
        ...
        if "insertion" in line:
            try:
                inserts = int(line.split()[0])
            except (IndexError, ValueError):
                pass
        if "deletion" in line:
            try:
                deletes = int(line.split()[0])
            except (IndexError, ValueError):
                pass
    return {"files_changed": files, "insertions": inserts, "deletions": deletes}


def run_pre_push_tests() -> Dict[str, Any]:
    """
    Run pytest on staged Python files.
    Returns dict with ok, stdout, stderr, returncode.
    """
    staged = get_staged_files()
    if not staged:
        return {"ok": True, "stdout": "No staged Python files to test", 
                "stderr": "", "returncode": 0, "files_tested": 0}
